...
namespace ALBAITAR_Softvet.Resources
{
    public partial class Vente_2 : Form
    {
        ...
        public Vente_2(int to_select_id)
        {
            ...
            //-----------------------
            clients = PreConnection.Load_data("SELECT *,CONCAT(`SEX`,' ',`FAMNME`,' ',`NME`) AS FULL_NME FROM tb_clients ORDER BY FAMNME;");
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            // insert_or_update_also_the_caisse_in_provisoire = true;
            //selected_fact_caisse_mnt = 0;
            ...
            if (factures != null)
            {
                ...
                {
                    IEnumerable<DataRow> rows = factures.AsEnumerable()
                                  .Where(row => row["REF"].ToString().Substring(3, 4) == DateTime.Now.Year.ToString());
                    if (rows != null)
                    {
                        if (rows.Count() > 0)
                        {
                            dds = rows.Max(row => int.Parse(row["REF"].ToString().Substring(8)));
                        }
                    }
        ...
        private void infos_save(int IS_PROVIS)
        {
            //----------
        ...
        private void verif_ref_exist()
        {
            ...
            int cnt = Is_New ? factures.AsEnumerable().Count(rw => rw["REF"].ToString() == reff) :
                (dataGridView1.SelectedRows.Count > 0 ? factures.AsEnumerable().Count(rw => rw["REF"].ToString() == reff && rw["ID"].ToString() != dataGridView1.SelectedRows[0].Cells["ID"].Value.ToString()) : 0);
        ...
        {
            if (dateTimePicker2_IsDropedDown)
            {
                dateTimePicker2_IsDropedDown = false;
                SendKeys.Send("{ENTER}");
            }
            verif_ref_exist();
        }
        ...
        private void comboBox1_TextUpdate(object sender, EventArgs e)
        {
            ...
            {
                decimal cc = Selected_facture_old_infos.Rows[0]["PAYED_MNT"] != DBNull.Value ? (decimal)Selected_facture_old_infos.Rows[0]["PAYED_MNT"] : 0;
                bool is_not_provis = (Selected_facture_old_infos.Rows[0]["IS_PROVIS"] != DBNull.Value ? (int)Selected_facture_old_infos.Rows[0]["IS_PROVIS"] : 1) != 1;
                if (!Is_New && cc != 0 && is_not_provis)
                {
                    if (!comboBox1.Text.Trim().Equals(Selected_facture_old_infos.Rows[0]["CLIENT_FULL_NME"]) && !has_asked_for_Transf_also_caisse)
                    {
                        has_asked_for_Transf_also_caisse = true;
                        Transf_also_caisse = MessageBox.Show("Voulez-vous rendre le montant " + ((decimal)Selected_facture_old_infos.Rows[0]["PAYED_MNT"]).ToString("N2") + " DA au " + dataGridView1.SelectedRows[0].Cells["CLIENT_FULL_NME"].Value + " -qu'il a payé precedement- ?", "Attention :", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes;
                    }
                }
                else
                {
            ...
            else
            {
                ...
                {
                    if (clients.Rows.Cast<DataRow>().Count(k => (k["FULL_NME"] != DBNull.Value ? k["FULL_NME"].ToString() : "").Equals(comboBox1.Text)) == 1)
                    {
                        groupBox3.Enabled = true;
                        comboBox1.SelectedValue = clients.Rows.Cast<DataRow>().Where(k => (k["FULL_NME"] != DBNull.Value ? k["FULL_NME"].ToString() : "").Equals(comboBox1.Text)).First()["ID"];
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            //----------------------
            Selected_facture_old_infos = null;
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                Selected_facture_old_infos = PreConnection.Load_data("SELECT * FROM tb_factures_vente WHERE `ID` = " + idd + ";");
                ...
                if (Selected_facture_old_infos != null)
                {
                    if (Selected_facture_old_infos.Rows.Count > 0)
                    {
                        ...
                        checkBox1.CheckedChanged += checkBox1_CheckedChanged;
                        numericUpDown2.Value = Selected_facture_old_infos.Rows[0]["PAYED_MNT"] != DBNull.Value ? (decimal)Selected_facture_old_infos.Rows[0]["PAYED_MNT"] : 0;
                        //selected_fact_caisse_mnt = Selected_facture_old_infos.Rows[0]["PAYED_MNT"] != DBNull.Value ? (decimal)Selected_facture_old_infos.Rows[0]["PAYED_MNT"] : -1;
                        pictureBox1.Visible = (int)Selected_facture_old_infos.Rows[0]["IS_PROVIS"] == 1;
                        //-----------------
                        for (int f = 1; f < 71; f++)
                        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (MessageBox.Show("Êtes-vous sûr de supprimer (" + dataGridView1.SelectedRows.Count + ") factures ?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    dataGridView1.SelectedRows.Cast<DataGridViewRow>().ForEach(row =>
                    {
                        ...
                        facts_nums += row.Cells["REF"].Value != null && row.Cells["REF"].Value != DBNull.Value ? (row.Cells["REF"].Value.ToString().Trim().Length > 0 ? ",'" + row.Cells["REF"].Value.ToString().Trim() + "'" : "") : "";
                    });
                    facts_nums = facts_nums.Length > 0 ? facts_nums.Substring(1) : facts_nums;
                    idx = idx.Substring(1);
                    //----------
                    bool regl_stock = MessageBox.Show("Retourner la quantité des produits au stock ?\n\n(Concerne juste les produits connus dans la base donné)", "Stock :", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes;
                    //bool regl_finance = MessageBox.Show("Révoquer-vous les droits monétique de cette(ces) facture(s) ?\n\n(S'il existent)", "Caisse :", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes;
                    if (regl_stock)
                    {
                        string cmd_tmp = "";
                        for (int f = 1; f < 71; f++)
                    ...
                    //if (regl_finance)
                    //{
                    //    PreConnection.Excut_Cmd_personnel("DELETE FROM tb_clients_finance WHERE FACT_NUM IN (" + facts_nums + ");", null, null);
                    //}
                    //else
                    //{
                    //    PreConnection.Excut_Cmd_personnel("UPDATE tb_clients_finance SET FACT_NUM = NULL WHERE FACT_NUM IN (" + facts_nums + ");", null, null);
                    //}
                    //--------
        ...
        private void factures_filtr()
        {
            if (dataGridView1.DataSource != null)
            {
                ...
                if (checkBox7.Checked) //Facture provisoire
                {
                    fltr += !string.IsNullOrWhiteSpace(fltr) ? " AND " : "";
                    fltr += "IS_PROVIS = 1";
                }
        ...
        {
            infos_save(comboBox4.SelectedIndex);
        }
        ...
        private void dataGridView1_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
            ...
            headerCellStyle.SelectionBackColor = headerCellStyle.BackColor = (dataGridView1.Rows[e.RowIndex].Cells["IS_PROVIS"].Value ?? "").ToString().Equals("1") ? panel2.BackColor : SystemColors.Control;
        }

        private void dateTimePicker2_DropDown(object sender, EventArgs e)
        {
            dateTimePicker2_IsDropedDown = true;
            // First CTRL+UP
            SendKeys.Send("^{UP}");
        ...
        {
            if (dataGridView1.Rows[e.RowIndex].Selected && Is_New)
            {
