...
{
    public partial class Anims_List_For_Vaccin : Form
    {
        ...
        {
            Add_Modif_Vaccin.Animm = new ListViewItem[listView2.Items.Count];
            for (int i = 0; i < listView2.Items.Count; i++)
            {
                Add_Modif_Vaccin.Animm[i] = listView2.Items[i];
            }
        ...
        private void Anims_List_Load(object sender, EventArgs e)
        {
            ...
            props = PreConnection.Load_data("SELECT tb1.ID,tb1.NME,tb1.CLIENT_ID,CONCAT(tb2.FAMNME, ' ', tb2.NME) AS CLIENT_FULL_NME,tb1.NUM_IDENTIF FROM tb_animaux AS tb1 LEFT JOIN tb_clients AS tb2 ON tb1.`CLIENT_ID` = tb2.ID ORDER BY NME;");
            if (props != null)
            {
                if (props.Rows.Count > 0)
                {
                    ...
                    if (Add_Modif_Vaccin.Animm2.Length > 0)
                    {
                        for (int dd = 0; dd < Add_Modif_Vaccin.Animm2.Length; dd++)
                        {
                            ListViewItem item = listView1.Items.Cast<ListViewItem>().Where(XX => XX.SubItems[0].Text == Add_Modif_Vaccin.Animm2[dd].SubItems[0].Text).FirstOrDefault();
                            listView1.Items.Remove(item);
                            items2.Remove(item);
                            listView2.ItemSelectionChanged -= listView2_ItemSelectionChanged;
                            listView2.Items.Add(item);
