    ...
    public partial class Animaux : Form
    {
        ...
        private void Load_selected_anim_fields()
        {
            ...
            if (poids_tbl != null)
            {
                ...
                {
                    var ttt = poids_tbl.AsEnumerable().Where(x => (x["ANIM_ID"] != DBNull.Value ? (int)x["ANIM_ID"] : -1) == (int)dataGridView1.SelectedRows[0].Cells["ID"].Value && (x["POIDS"] != DBNull.Value ? (double)x["POIDS"] : -1) > 0);
                    if (ttt.Any())
                    {
...
(checkBox1.Checked ? textBox5.Text : "")
        ...
        private void new_anim(bool btn_clicked)
        {
            ...
            comboBox3.Text = "--";
            foreach (Control ctrl in Infos_Panel.Controls)// tabPage1.Controls)
            {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (MessageBox.Show("Vous étes sures de supprimer [" + dataGridView1.SelectedRows.Count + "] animaux ?\n\nAttention: Tous " + (dataGridView1.SelectedRows.Count == 1 ? "ses" : "leurs") + " infos associées seront supprimés (Laboratires, Visies, Agenda ...).", "Confirmer :", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    fff = fff.Substring(1);
                    PreConnection.Excut_Cmd_personnel("DELETE FROM tb_animaux WHERE ID IN (" + fff + ")", null, null);
...
richTextBox1.Text
            ...
            if (dataGridView2.SelectedCells.Count > 0 && button11.Enabled)
            {
                if (MessageBox.Show("Vous-étes sur de faire la suppression de (" + dataGridView2.SelectedRows.Count + ") visites ?", "Confirmer :", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    ...
                    if (ids.Length > 0)
                    {
                        ...
                        PreConnection.Excut_Cmd_personnel("DELETE FROM tb_visites WHERE ID IN (" + ids + ")", null, null);
                        //-----------
        ...
        //        dataGridView2.ClearSelection();
        //        dataGridView2.SelectionChanged += dataGridView2_SelectionChanged;
        //        dataGridView2.Rows.Cast<DataGridViewRow>().Where(xx => (int)xx.Cells["ID_VISITE"].Value == firstOpenVisitID).ToList().ForEach(dx =>
        //        {
        //            dx.Selected = true;
        //            if (dataGridView2.DisplayedRowCount(false) < dataGridView2.RowCount) { dataGridView2.FirstDisplayedScrollingRowIndex = dx.Index; }
        //        });

        //        firstOpenVisitID = -1;
        //    }
        //    else if (firstOpenVisitID == -2)
        //    {
        ...
        private async void SelectVisitID()
        {
            ...
            if (visite_idd > 0)
            {
                ...
                dataGridView2.ClearSelection();
                dataGridView2.SelectionChanged += dataGridView2_SelectionChanged;
                dataGridView2.Rows.Cast<DataGridViewRow>().Where(xx => (int)xx.Cells["ID_VISITE"].Value == visite_idd).ToList().ForEach(dx =>
                {
                    dx.Selected = true;
                    if (dataGridView2.DisplayedRowCount(false) < dataGridView2.RowCount) { dataGridView2.FirstDisplayedScrollingRowIndex = dx.Index; }
                });
        ...
        private void dataGridView3_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (prev_col_idx_dgv3 == dataGridView3.Columns["DELETE"].Index && prev_rw_idx_dgv3 != dataGridView3.NewRowIndex)
            {
                ...
                {
                    PreConnection.Excut_Cmd(3, "tb_poids", null, null, "ID = @P_ID", new List<string> { "P_ID" }, new List<object> { dataGridView3.Rows[prev_rw_idx_dgv3].Cells["IDD"].Value });
                    poids_tbl = PreConnection.Load_data("SELECT * FROM tb_poids ORDER BY DATETIME ASC;");
                    load_poids();
        ...
        private void dataGridView3_Scroll(object sender, ScrollEventArgs e)
        {
            if (dataGridView3.Rows.Count > 1)
            {
                if (prev_col_idx_dgv3 == dataGridView3.Columns["DATETIMEE"].Index)
                {
                    ...
                    dateTimePicker5.Visible = prev_rw_idx_dgv3 >= dataGridView3.FirstDisplayedScrollingRowIndex && prev_rw_idx_dgv3 < (dataGridView3.FirstDisplayedScrollingRowIndex + dataGridView3.DisplayedRowCount(false));
                }
                else if (prev_col_idx_dgv3 == dataGridView3.Columns["POIDS"].Index)
                {
                    ...
                    {
                        goo &= dataGridView3.CurrentCell.ColumnIndex == prev_col_idx_dgv3;
                    }
                    ...
                    if (goo)
                    {
                        ...
                        numericUpDown1.Visible = prev_rw_idx_dgv3 >= dataGridView3.FirstDisplayedScrollingRowIndex && prev_rw_idx_dgv3 < (dataGridView3.FirstDisplayedScrollingRowIndex + dataGridView3.DisplayedRowCount(false));
                    }
            ...
            {
                prev_rw_idx_dgv3 = prev_col_idx_dgv3 = -1;
                dateTimePicker5.Visible = numericUpDown1.Visible = false;
        ...
        private void Load_malad_2()
        {
            if (tabControl1.SelectedTab == tabPage4 && dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (filtred_maladies_tbl != null)
                {
                    ...
                    {
                        var selectedID = dataGridView1.SelectedRows[0].Cells["ID"].Value;
                        if (selectedID != null && int.TryParse(selectedID.ToString(), out int id))
                        {
                            var ggg = filtred_maladies_tbl.AsEnumerable().Where(F => F.Field<int>("ANIM_ID") == id);
                            if (ggg.Any())
                            {
        ...
        private void reload_cbx6_data()
        {
            ...
            comboBox8.DataSource = malad_lst;
        ...
        private void dataGridView4_SelectionChanged(object sender, EventArgs e)
        {
            if (dataGridView4.SelectedRows.Count > 0)
            {
                ...
                dateTimePicker8.Value = dateTimePicker9.Value = DateTime.Now;
                comboBox8.Text = string.Empty;
                comboBox9.SelectedIndex = 0;
                //-------------------------
                Is_New_Maladie = false;
                pictureBox4.Image = Properties.Resources.MODIF_003;
                DataGridViewRow row = dataGridView4.SelectedRows[0];
                dateTimePicker8.Value = (DateTime)row.Cells["START_DATE_MALAD"].Value;
                if (row.Cells["ESTIM_END_DATE_MALAD"].Value != DBNull.Value)
                {
                    checkBox4.Checked = true;
                    dateTimePicker9.Value = (DateTime)row.Cells["ESTIM_END_DATE_MALAD"].Value;
                }
                comboBox8.Text = row.Cells["MALAD_NME"].Value != DBNull.Value ? (string)row.Cells["MALAD_NME"].Value : "";
                string levl = row.Cells["MALAD_LEVEL"].Value != DBNull.Value ? (string)row.Cells["MALAD_LEVEL"].Value : "";
                if (!string.IsNullOrEmpty(levl) && comboBox9.Items.Contains(levl)) { comboBox9.SelectedItem = levl; }
        ...
        private void button16_Click(object sender, EventArgs e)
        {
            ...
            Is_New_Maladie = true;
            pictureBox4.Image = Properties.Resources.NOUVEAU_003;
            ...
            dataGridView4.SelectionChanged -= dataGridView2_SelectionChanged;
            dataGridView4.ClearSelection();
            dataGridView4.SelectionChanged += dataGridView2_SelectionChanged;


            if (sender != null) { comboBox8.Focus(); }
        }
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            ...
            if (checkBox4.Checked)
            {
                ...
                panel4.Visible = dateTimePicker9.Value.Date < dateTimePicker8.Value.Date;
            }
            comboBox8.BackColor = string.IsNullOrWhiteSpace(comboBox8.Text) ? Color.LightCoral : Color.White;
            pret &= !string.IsNullOrWhiteSpace(comboBox8.Text);


            if (pret)
            {
                if (Is_New_Maladie) //INSERT 
                {
                    PreConnection.Excut_Cmd(1, "tb_maladies", new List<string> {
                       ...
                       {
                        dataGridView1.SelectedRows[0].Cells["ID"].Value,
                        dateTimePicker8.Value,
                        comboBox8.Text,
                        comboBox9.Text,
                        checkBox4.Checked ? dateTimePicker9.Value : (object)DBNull.Value
                       }, null, null, null);
                }
                else //UPDATE
                {
                    PreConnection.Excut_Cmd(2, "tb_maladies", new List<string> { "START_DATE", "MALAD_NME", "MALAD_LEVEL", "ESTIM_END_DATE" }, new List<object>
                    {
                        ...
                        comboBox9.Text,
                        checkBox4.Checked ? dateTimePicker9.Value : (object)DBNull.Value
                    }, "ID = @PP_IDD", new List<string> { "PP_IDD" }, new List<object> { dataGridView4.SelectedRows[0].Cells["ID_MALAD"].Value });
                }
        ...
        {
            dateTimePicker9.Enabled = checkBox4.Checked;
            panel4.Visible = false;
        }
        ...
        private void button17_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Vous-étes sur de faire la suppression de (" + dataGridView4.SelectedRows.Count + ") maladie ?", "Confirmer :", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                ...
                if (ids.Length > 0)
                {
                    ...
                    PreConnection.Excut_Cmd_personnel("DELETE FROM tb_maladies WHERE ID IN (" + ids + ")", null, null);
                    Load_malad_1();
                }
        ...
        {
            string vv = dataGridView3.Rows[e.RowIndex].Cells["POIDS"].Value != null && dataGridView3.Rows[e.RowIndex].Cells["POIDS"].Value != DBNull.Value ? dataGridView3.Rows[e.RowIndex].Cells["POIDS"].Value.ToString() : "";
            decimal dd = 0;
            decimal.TryParse(vv, out dd);
